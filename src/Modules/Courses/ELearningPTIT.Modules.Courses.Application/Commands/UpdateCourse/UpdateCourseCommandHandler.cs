...
) : ICommandHandler<UpdateCourseCommand, CourseDto>
{
    public async Task<CourseDto> HandleAsync(UpdateCourseCommand command)
    {
        ...
        {
            var newCategory = await categoryRepository.GetAsync(command.CategoryId);
            if (newCategory == null)
            {
            ...
            var oldCategory = await categoryRepository.GetAsync(course.CategoryId);
            if (oldCategory != null)
            {
                oldCategory.CourseCount--;
                await categoryRepository.ReplaceAsync(oldCategory);
            }

            newCategory.CourseCount++;
            await categoryRepository.ReplaceAsync(newCategory);
        }
