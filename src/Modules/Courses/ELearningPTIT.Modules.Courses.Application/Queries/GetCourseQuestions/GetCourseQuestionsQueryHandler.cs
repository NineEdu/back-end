...
) : IQueryHandler<GetCourseQuestionsQuery, List<QuestionDto>>
{
    ...
    {
        var questions = await questionRepository.QueryAsync(x => x.CourseId == query.CourseId, cancellationToken);
        return questions.OrderByDescending(x => x.CreatedAt).Adapt<List<QuestionDto>>();
    }
}
